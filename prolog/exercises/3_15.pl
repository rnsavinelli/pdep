%¡Uno más para poner en práctica todo lo que aprendimos en esta lección!
...
zonaDesierta(Zona) :-
  ...
  forall(quedaEn(Lugar, Zona), nadieQuiereIr(Lugar)).

nadieQuiereIr(Lugar) :-
  not(alguienQuiereIr(Lugar)).

alguienQuiereIr(Lugar) :-
  quiereIr(_, Lugar).
